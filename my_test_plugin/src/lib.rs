...
impl Plugin for MyTestPlugin {
    ...
    fn init(&self) {
        ...
        HOOK_REGISTRY.lock().unwrap().action_add_route.push(Box::new(ActionAddRoute{priority: 10}));
        HOOK_REGISTRY.lock().unwrap().filter_the_content.push(Box::new(FilterTheContent{priority: 0}));
    }
}

pub struct ActionAddRoute {
    priority: i32
}

impl Hook for ActionAddRoute {
    fn action_add_route(&self){
        let r1 = Route::new("GET", "/one", 9999, do_one);
        let r2 = Route::new("GET", "/two", 10000, do_two);
        let r3 = Route::new("GET", "/my_test_plugin/public/*", 9998, do_three);

        ROUTE_REGISTRY.lock().unwrap().add_route(r1);
        ROUTE_REGISTRY.lock().unwrap().add_route(r2);
        ROUTE_REGISTRY.lock().unwrap().add_route(r3);
    }
...
fn do_three(request: &Request) -> Response {
    if let Some(request) = request.remove_prefix("/my_test_plugin/public") {
        return match_assets(&request, "../my_test_plugin/public"); // the following 2 functions from rouille had to be overwritten, to be able to go a directory out... (out of the my_test_site folder)
    }
...
fn match_assets<P: ?Sized>(request: &Request, path: &P) -> Response
    where P: AsRef<Path>
{
...
// this needs to be overwritten as well, since it's not a pub function and can't be called with "rouille::extension_to_:mime"
